...
- A supplied folder is identified, not assumed to be home. Adapters register root signatures (exact child names that identify a tool root or store under any name, such as `opencode.db` or `sessions` beside `config.toml`) and history-name evidence (file-name patterns that tell Claude Code's `projects` from Command Code's, and Codex's `sessions` from other tools'). A history folder reached without its tool's own parent must fit that evidence, checked by listing it and at most four subfolders, names only. Folders from the folder chooser go through the same recognition over the chooser's file list.
- Discovery never lists the dropped folder or any folder on the way to a location. It lists only inside a found or candidate history folder, bounded by the adapter's depth and extension, and takes file sizes from the File the browser hands over without reading it. A privacy contract test checks every probe against `registeredProbePaths()` and every listing against the found folders, in unit tests and against Chromium's real entries.
