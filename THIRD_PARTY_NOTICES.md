...
| --- | --- |
| Geist fonts | SIL OFL 1.1. Preserve copyright and license with distributed fonts. The complete upstream text is served at `/licenses/geist.txt` and stored in [apps/web/public/licenses/geist.txt](apps/web/public/licenses/geist.txt). Fonts remain OFL; application source remains AGPL. Observe reserved-name restrictions when modifying fonts. |
